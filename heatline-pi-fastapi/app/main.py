...
from .db import init_db, insert_control_log, insert_event, list_control_logs, list_events
from .models import (
    ApiResponse,
    CommandPayload,
    DeviceInfoResponse,
...
def get_events(limit: int = Query(default=20, ge=1, le=200)):
    return ApiResponse(data=list_events(limit=limit))
...
def get_control_logs(limit: int = Query(default=20, ge=1, le=200)):
    return ApiResponse(data=list_control_logs(limit=limit))
